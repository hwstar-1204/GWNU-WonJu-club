...
urlpatterns = [
    ...
    path('board_posts/<str:club_name>/<str:category>/<str:order>/', BoardPostListView.as_view({'get': 'list'}), name='board_posts'),  # 모든 게시글 불러오기 (페이지네이션 적용)
    ...
    path('post_recommend/<int:post_id>/', PostRecommendView.as_view(), name='post_recommend'),  # 특정 게시글 추천하기
